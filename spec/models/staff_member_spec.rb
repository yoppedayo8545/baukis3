...
RSpec.describe StaffMember, type: :model do
  ...
  describe "値の正規化" do
    ...
    example "family_name_kanaに含まれる半角カナを全角に変換" do
  ...
  describe "バリデーション" do
    ...
    example "漢字を含むgiven_nameは有効" do
      member = build(:staff_member, given_name: "試験" )
      expect(member).to be_valid
    end

    example "ひらがなを含むgiven_nameは有効" do
      member = build(:staff_member, given_name: "てすと" )
      expect(member).to be_valid
    end

    example "カタカナを含むgiven_nameは有効" do
      member = build(:staff_member, given_name: "テスト" )
      expect(member).to be_valid
    end

    example "アルファベットを含むgiven_nameは有効" do
